...
function buildYleCDNImageUrl(base, imsId, extension) {
  ...
  const defaultBase =
    "https://images.cdn.yle.fi/image/upload/f_auto,fl_progressive/q_100/h_" + 1560 *zoomFactor + ",c_fill/";
  ...
  return new Promise((resolve, reject) => {
    ...
    imgObj.image.onload = function () {
      ...
      document.getElementById("zoomedslider").min = -Math.abs(
        imgObj.image.naturalHeight - 1560
      );
...
download_img = function (el) {
  ...
  aDownloadLink.download = "Juliste" + "." + timeStamp +"."+ currentPodd;
