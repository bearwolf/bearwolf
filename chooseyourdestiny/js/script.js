...
function buildYleCDNImageUrl(base, imsId, extension) {
  ...
  const defaultBase =
    "https://images.cdn.yle.fi/image/upload/f_auto,fl_progressive/q_100/w_" + 4000*zoomFactor + ",c_fill/";
  ...
  return [
    {
      type: "image",
      url: selectedPodd,
      rect: { x: 0, y: 0 + +heightOffset, w: 4000, h: 2250 },
      drawOpts: {
        fillStyle: "black",
...
download_img = function (el) {
  ...
  aDownloadLink.download = "Poddtool" + "." + timeStamp +"."+ currentPodd;
...
function playMIDI() {
  MIDI.loadPlugin({
      ...
      instrument: "acoustic_grand_piano", // Välj instrumentet du vill använda
      onprogress: function(state, progress) {
          console.log(state, progress);
      },
      onsuccess: function() {
          MIDI.Player.loadFile("midi/kombat.mid", MIDI.Player.start);
